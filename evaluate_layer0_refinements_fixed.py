#!/usr/bin/env python3
"""
Layer 0 Refinements Evaluation Script - FIXED VERSION
Fixes resolution mismatch and test configuration issues
"""
...
def resize_image_to_384(img_224):
    """Resize 224x224 image to 384x384 as expected by the model."""
    # Simple resize - could use more sophisticated methods if needed
    img_384 = cv2.resize(img_224, (384, 384), interpolation=cv2.INTER_LINEAR)
    return img_384

def resize_keypoints_224_to_384(keypoints_224):
    """Scale keypoints from 224x224 to 384x384 coordinate space."""
    scale_factor = 384.0 / 224.0
    keypoints_384 = keypoints_224.copy()
    # Only scale valid keypoints
    valid_mask = (keypoints_224[:, 0] > 0) & (keypoints_224[:, 1] > 0)
    keypoints_384[valid_mask] = keypoints_224[valid_mask] * scale_factor
    return keypoints_384

def resize_keypoints_384_to_224(keypoints_384):
    """Scale keypoints from 384x384 back to 224x224 coordinate space."""
    scale_factor = 224.0 / 384.0
    keypoints_224 = keypoints_384.copy()
    # Only scale valid keypoints
    valid_mask = (keypoints_384[:, 0] > 0) & (keypoints_384[:, 1] > 0)
    keypoints_224[valid_mask] = keypoints_384[valid_mask] * scale_factor
    return keypoints_224
...
def main():
    """Main evaluation function with Layer 0 refinements - FIXED."""
    
    print("="*80)
    print("LAYER 0 REFINEMENTS - FIXED VERSION (384x384)")
    print("="*80)
    ...
    print(f"Model expects: 384x384 images (will resize from 224x224)")
    
    # Get landmark information
    landmark_names = cephalometric_dataset_info.landmark_names_in_order
    landmark_cols = cephalometric_dataset_info.original_landmark_cols
    
    # Prepare training keypoints for median filtering (in 224x224 space)
    ...
    # Add proper test_cfg to baseline
    cfg_baseline = cfg.copy()
    cfg_baseline.model.test_cfg = dict(
        flip_test=False,
        shift_heatmap=False,
        use_dark=False
    )
    ...
    model_baseline = init_model(cfg_baseline, checkpoint_path, 
                               device='cuda:0' if torch.cuda.is_available() else 'cpu')
    
    for idx, row in test_df.iterrows():
        # Get image (224x224)
        img_array_224 = np.array(row['Image'], dtype=np.uint8).reshape((224, 224, 3))
        
        # Resize to 384x384 for model
        img_array_384 = resize_image_to_384(img_array_224)
        ...
        for i in range(0, len(landmark_cols), 2):
            ...
            y_col = landmark_cols[i+1]
            if x_col in row and y_col in row and pd.notna(row[x_col]) and pd.notna(row[y_col]):
                gt_keypoints_224.append([row[x_col], row[y_col]])
            else:
                gt_keypoints_224.append([0, 0])
        gt_keypoints_224 = np.array(gt_keypoints_224)
        
        # Prepare bbox for 384x384
        bbox = np.array([[0, 0, 384, 384]], dtype=np.float32)
        
        # Run inference
        results = inference_topdown(model_baseline, img_array_384, bboxes=bbox, bbox_format='xyxy')
        
        if results and len(results) > 0:
            pred_keypoints_384 = results[0].pred_instances.keypoints[0]
            # Convert predictions back to 224x224 space for evaluation
            pred_keypoints_224 = resize_keypoints_384_to_224(pred_keypoints_384)
            
            results_baseline['pred'].append(pred_keypoints_224)
            results_baseline['gt'].append(gt_keypoints_224)
    ...
    for idx, row in test_df.iterrows():
        img_array_224 = np.array(row['Image'], dtype=np.uint8).reshape((224, 224, 3))
        img_array_384 = resize_image_to_384(img_array_224)
        gt_keypoints_224 = results_baseline['gt'][idx]  # Reuse GT from baseline
        
        bbox = np.array([[0, 0, 384, 384]], dtype=np.float32)
        results = inference_topdown(model_dark, img_array_384, bboxes=bbox, bbox_format='xyxy')
        
        if results and len(results) > 0:
            pred_keypoints_384 = results[0].pred_instances.keypoints[0]
            pred_keypoints_224 = resize_keypoints_384_to_224(pred_keypoints_384)
            
            results_dark['pred'].append(pred_keypoints_224)
            results_dark['gt'].append(gt_keypoints_224)
    ...
    for idx, row in test_df.iterrows():
        img_array_224 = np.array(row['Image'], dtype=np.uint8).reshape((224, 224, 3))
        img_array_384 = resize_image_to_384(img_array_224)
        gt_keypoints_224 = results_baseline['gt'][idx]
        
        bbox = np.array([[0, 0, 384, 384]], dtype=np.float32)
        results = inference_with_flip_test(model_dark, img_array_384, bbox)
        
        if results and len(results) > 0:
            pred_keypoints_384 = results[0].pred_instances.keypoints[0]
            pred_keypoints_224 = resize_keypoints_384_to_224(pred_keypoints_384)
            
            results_flip['pred'].append(pred_keypoints_224)
            results_flip['gt'].append(gt_keypoints_224)
    ...
    # Save results
    output_dir = os.path.join(work_dir, "layer0_refinements_fixed")
    ...
    plt.title('Layer 0 Refinements - Progressive Improvement (Fixed 384x384)')
    ...
    print("\n⚠️  Note: These results are in 224x224 pixel space for comparison with previous results.")
    print("The model internally processes at 384x384 resolution.")
